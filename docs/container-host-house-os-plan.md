...
- The first approved acceptance attempt on 2026-07-22 passed file, image,
  ...
  first-run listener as `1000:1000`; Compose now declares that identity. A
  fourth attempt made
...
- The corrected fifth deployment completed at `2026-07-22T21:03:05Z`. It
  passed loopback-only first start, account and managed-object bootstrap, scoped
  ntfy access, LAN cutover, idempotent reconciliation, exact image and identity
...
- The first `stacks/monitoring/` bundle is implemented and Compose-valid with
  ...
  Exact-image validation and the approved cutover passed on 2026-07-21.
...
- The approved `retire_legacy_monitoring_files` action removed the three
  verified inert proof-of-concept files and their empty directory using only
  non-recursive deletion. Final targeted inventory passed with no findings.
...
- [x] Monitoring secret stored outside Git and never included in reports or
  action history; apply the same pattern to future stacks.
